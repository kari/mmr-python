...
For reference, see http://jmlr.csail.mit.edu/papers/volume12/weng11a/weng11a.pdf
"""
...
def probs(teams):
    ...
    for i in range(k):
        ...
        for q in [q for q in range(k) if q != i]:
            ...
            phat_iq = math.exp(m_i / c_iq) / (
                math.exp(m_i / c_iq) + math.exp(m_q / c_iq)
            )
...
def update(teams, r):
    ...
    for i in range(k):
        ...
        for q in [q for q in range(k) if q != i]:
            ...
            phat_qi = math.exp(m_q / c_qi) / (
                math.exp(m_q / c_qi) + math.exp(m_i / c_qi)
            )
            # phat_qi = 1-phat_iq
